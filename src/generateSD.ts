...
import {  interactionType,
          ...
          typeToOp,
          typeToTd,
          singToPlural} from "../definitions/enums"
...
// determine filePaths
const mashupTemplatePath = "./definitions/mashup-template.json"

export default function generateSD(umlMashupLogic: SQSD.mashupLogic, TdsFileContent: string) {

  // Parse Tds
  const fileContentsTds: SQSD.tdTemplate[] = JSON.parse(TdsFileContent)

  // ------------ Program -------------

    let mashupSD = initSd(mashupTemplatePath, fileContentsTds, umlMashupLogic.name)
...
function initSd(
  ...
  tds: SQSD.tdTemplate[],
  mashupName?: string
): SQSD.sdTemplate & {actions; properties; events; base; id; functions; variables} {
  const mashupTemplateProto: SQSD.sdTemplate & {
  ...
  mashupTemplateProto.title = mashupName ?? genName
 ...
 */
function fillSd(
  umlStrct: SQSD.mashupLogic,
  protocolSelection: SQSD.validProtocols,
  tds: SQSD.tdTemplate[],
  mashupTemplateProto: SQSD.sdTemplate & {actions; properties; events; base; id; functions; variables}
): SQSD.sdTemplate & {actions; properties; events; base; id; functions; variables} {
  const isAdded: SQSD.interactionAll[] = []
  ...
  function parseStrct( strctArray: SQSD.structureEl[]) {
   ...
   */
  function addForms(umlIntrcts: SQSD.interactionAll[]) {
        ...
        tds.forEach(td => {
          ...
          if (noSpaceTitle === interact.to) {
            ...
            if (interact.type in interactionType) {
              ...
              const nextOp: string = typeToOp[interactionType[interact.type]].toLowerCase()
              ...
              let formCandidates: SQSD.tdForms[] = td[ttDIntr][interact.name].forms.filter(
                candidate => {
              ...
              formCandidates = formCandidates.map(el => {
                if(el.href.includes("://")) {return el}
                  ...
                  el.href = td?.base + el.href
                  return el
                }
              })

              // filter for forms that have no href included in protocol_selection
              formCandidates = formCandidates?.filter( candidate => {
                const candidateProtocol = candidate.href.split("://", 2).shift() ?? "null"
              ...
              formCandidates.sort( (a, b) => {
                const aProt = a.href.split("://", 2).shift() ?? "null"
                const bProt = b.href.split("://", 2).shift() ?? "null"
              ...
              if (firstForm) {
                ...
                const addToForm: SQSD.tdForms = mashupTemplateProto.things[noSpaceTitle][ttDIntr][interact.name].forms.filter(
                  (possibleForm: SQSD.tdForms, idx: number) => {
                    let fit = true
              ...
              if(!td[ttDIntr][interact.name].forms[interact.formRef].href.includes("://")) {
                baseProto = td?.base + td[ttDIntr][interact.name].forms[interact.formRef].href
              }
...
function parseStrctWrapper(
  wholeStrctArray: SQSD.mashupLogic,
  mashupTemplate: SQSD.sdTemplate & {actions; properties; events; base; id; functions; variables}
) {
   ...
   */
  function parseStructure(strctArray: SQSD.structureEl[]): SQSD.pathEl[] {

    const pathProto: SQSD.pathEl[] = []
    ...
    strctArray.forEach( strct => {
      switch (strct.type) {
        ...
        case structureType.case:
          ...
          const structIfconv = (inS: SQSD.comparison) => {
            let outS: SQSD.ifWord
   ...
   */
  function parseReceiveInteractions(intrctArray: SQSD.interactionReceive[]) {
    const pathPrototype: SQSD.pathInteractReceive[] = []
    ...
    intrctArray.forEach( currentIntrct => {
      ...
      pathPrototype.push({
        ...
        op: typeToOp[interactionType[currentIntrct.type]].toLowerCase(),
        set
      })
   ...
   */
  function parseSendInteractions(intrctArray: SQSD.interactionSend[]) {
    const pathPrototype: SQSD.pathInteractSend[] = []
  ...
  function genFormRef( Intrct: SQSD.interactionAll ) {
