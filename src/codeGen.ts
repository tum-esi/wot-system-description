...
import {  loopType,
          structureType,
          interactionType,
          typeToOp} from "../definitions/enums"
...
export default function generateTS(SD: SQSD.sdTemplate, mashupLogic: SQSD.mashupLogic, fName: string) {
    ...
    const base = generateIndexJS(SD, fName)
    ...
    function parsePath(inPath: SQSD.structureEl[]) {
        ...
        function parsePathAtomic(inEl: SQSD.structElAtomic) {
            ...
            const hasDataPush = inEl.receiveIntrcts.some(
                rec => (rec.type === interactionType.observe || rec.type === interactionType.subscribe)
            )
            if (hasDataPush) {
            ...
            inEl.sendIntrcts.forEach( snd => {
                ...
                let inpost
                const fIntr = (snd.type === interactionType.write) ? "writeProperty" : "invokeAction"
            ...
            inEl.receiveIntrcts.forEach( rec => {
                ...
                if (rec.type === interactionType.read || rec.type === interactionType.invoke) {
                    proto += `${rec.set ? "" : "let"} ${varname} = this.consumed_things["${rec.to}"].` + fIntr + `("${rec.name}")`
                    waits.push(`${varname} = await ${varname}`)
                } else if (rec.type === interactionType.subscribe || rec.type === interactionType.observe) {
